...
%     end
     ...
     %testcase_number = 22; %temporary test case for skagit river which sets elevation to zero

      testcase_number = 23;  %year 2008, skgait river test with elevation set to MSL-NAVD88=0.715(m) based on Cresant harbor MSL=2.687 NAVD=1.29 (m)
...
%
			   ...
			   %testcase number 22, redo test 8 or 9 with zeta=0 and keep everything else,
                        ...
                        if(testcase_number==23)
                          %testcase number 23, set zeta = 0.715 (m) (Cresant Harbor MSL-NAVD88) + SLR (0.46m)
                           zeta=(0.715+0.46)*ones(size(zeta));
                        end
   ...
   for it_global=1:t_int:NT_global
       ...
       %read matlab file of the data saved above
		...
		if(testcase_number==22)
                ...
                if(testcase_number==23)
                  %testcase number 23, set zeta = 0.715 (m) (Cresant Harbor MSL-NAVD88) + SLR (0.46m for 2070)
                  zeta=(0.715+0.46)*ones(size(zeta));
                end
...
%                      s1_1(:,:)=s1(:,:); 
		 ...
		 %debug-- cancel smean at this time
		    ...
		    Smax=30.0;					    %maximum salinity or tracer concentration 
