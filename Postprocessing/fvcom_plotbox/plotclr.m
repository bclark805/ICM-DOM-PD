function plotc(x,y,v,marker,msz,cmin,cmax)
...
for nc=1:size(map,1)
    ...
    plot3(x(iv),y(iv),v(iv),marker,'MarkerSize',msz,'color',map(nc,:),'markerfacecolor',map(nc,:))
end
%fill below miv using color map(1,:)
iv=find(v<=miv);
if(length(iv)>0)
     plot3(x(iv),y(iv),v(iv),marker,'MarkerSize',msz,'color',map(1,:),'markerfacecolor',map(1,:))
end
%fill above mav using color map(end,:)
iv=find(v>mav);
if(length(iv)>0)
     plot3(x(iv),y(iv),v(iv),marker,'MarkerSize',msz,'color',map(end,:),'markerfacecolor',map(end,:))
end
hold off
