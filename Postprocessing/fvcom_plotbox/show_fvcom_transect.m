%show_fvcom_transect.m
%
%matlab tool to read the history outputs from fvcom model and then resave into matlab format for later use
%calculate statistics, plot vertical transects and statistics of flow across the transect
% 
% Also calculates transport through the transect (trracer, volume and fresh water flux)
%
...
%      if(isempty(PSFVM_ROOT))
%               display(['This program requires environmental variable PSFVM_ROOT to be set']);
%               display(['Please check the startup.m.template program in main directory']);
%               error('PSFVM_ROOT not setup yet');
%      else
%               if(~exist(PSFVM_ROOT,'dir'))
%                  error(['Oops, cannot find PSFVM_ROOT directory: ' PSFVM_ROOT]);
%               end
%      end

     %temporary directory name 
     tmpdirname='./__temp__dir__/';
     if(~exist(tmpdirname,'dir'))
        eval(['!mkdir -p ' tmpdirname]);
     end

     psm_mat_grid_file_tmp=['/Users/bclark/chesfvm-new/cpb/forcing/grid/v3/v3_2/cpb_v3_2_coarse.mat'];
     psm_mat_grid_file=input(['Please give the full path and file name of the grid file in matlab format (e.g. ' psm_mat_grid_file_tmp ')'],'s');

     if(exist(psm_mat_grid_file,'file'))  %if there is grid in matlab format, then load it dirrectly
	       ...
	       %grid with x y and depth (find x, y and depth of each node)
		gfile_node_xyd=input('please give the file name of node x, y and depth (e.g. psm_xy_depth_node.dat):','s');
		if(~exist(gfile_node_xyd,'file'))
		   error(['Oops can not find file :' gfile_node_xyd]);
		end

	       %gird with lat lon and depth (lat, lon and depth of each node)
	%	gfile_node_lld=input('please give the file anme of node lon, lat and depth (e.g. psm_lat_lon_depth_node.dat):','s');
	%	if(~exist(gfile_node_lld,'file'))
	%	   error(['Oops can not find file :' gfile_node_lld]);
	%	end

	       %grid with x y of each cell
	%	gfile_elem_xy=input('please give the file name of element x y coordinates (e.g. psm_xy_cell.dat):','s');
	%	if(~exist(gfile_elem_xy,'file'))
	%	   error(['Oops can not find file :' gfile_elem_xy]);
	%	end

	       %grid with with element to node mapping
		gfile_e2n=input('please give the element to node mapping file (e.g. psm_elem_to_node_mapping.dat):','s');
		if(~exist(gfile_e2n,'file'))
		   error(['Oops can not find file :' gfile_e2n]);
		end

	       %load x,y and depth of nodes
		 xyd_n=load(gfile_node_xyd); 
		 x_n=xyd_n(:,2);                  %x coordinate of nodes
		 y_n=xyd_n(:,3);                  %y coordinate of nodes

	       %load lat, lon and depth of nodes
%		 lld_n=load(gfile_node_lld);

                lld_n=load(gfile_node_xyd);       %caculate lat and lon internally instead

%		 lon_n=lld_n(:,2);                %longitude of nodes 
%		 lat_n=lld_n(:,3);                %latitude of nodes

               %calculate lon_n, lat_n from xy using utm2deg

	                 display('need to calculate lat lon from utm coordinate, please find UTM zone name from here: ');
	                 display('http://whatutmzoneamiin.blogspot.com/2011/03/what-utm-zone-am-i-in.html');
	                 utmzone=input('please input utm zone name (e. g. for Seattle, it is 10 T ):','s');
                         utmzones=[];
                         for i=1:length(x_n)
                            utmzones=[utmzones;utmzone];
                         end
	                 [lat_n,lon_n]=utm2deg(x_n,y_n,utmzones); 
	                 lat_n=reshape(lat_n,[length(lat_n),1]);  %make sure it is column vector
	                 lon_n=reshape(lon_n,[length(lon_n),1]);
	                 lld_n(:,2)=lon_n;
	                 lld_n(:,3)=lat_n;

		 h_n=lld_n(:,4);                  %depth of nodes

	       %load element to node mapping
		 e2n=load(gfile_e2n);

               %load x,y of cell
        %        xy_e=load(gfile_elem_xy);
        
               %calculate x,y of cell internally instead
                 xy_e=nan*zeros([size(e2n,1),2]);   
                 for ie=1:size(xy_e,1)
                      xy_e(ie,1)=mean(x_n(e2n(ie,2:4)));  %take mean of x of the nodes of the element
                      xy_e(ie,2)=mean(y_n(e2n(ie,2:4)));  %take mean of y of the nodes of the element
                 end

               %save the grid information
		 save('psm_grid.mat','xyd_n','lld_n','h_n','xy_e','e2n','-mat');   %save the grid information
     end 
     ...
     %obtain history output file (netcdf or medm)
             ...
             his_dir=input(['Please give the directory of the history output file of fvcom', ...
                             '(e.g. /home/wenlong/mypsfvm/trunk/example/pugetsound/run/outputs/netcdf):'],'s');

             if(~exist(his_dir,'dir'))
               error(['Oops, history output file dir:' his_dir ' does not exist!']);
             end

             hfile_prefix=input('Please give the history file name prefix (e.g. psm or psm_sim):','s');
             hfile_ext   =input('Please give the history file name extension (e.g. .nc or .dat):','s');
             his_Nz      =input('Please give number of vertical layers in history output (e.g. 10):');
             his_fn_start=input('Please give starting file number of history outputs (e.g. 1):');
             his_fn_end  =input('Please give ending file number of history outputs (e.g. 1000):');
             his_fn_int  =input('Please give interval of file processing (e.g. 10 for every 10 files):');
          ...
          %A typical medm file contains the following information :
               ...
               %     WRITE(1) IINTT,NGL,MGL,THOUR    !Internal time step, number of elements, number of nodes and time in hours
               %
               ...
               %           DO I=1,M
               %              WRITE(1) EL(I),(T1(I,K),S1(I,K),RHO1(I,K),K=1,KBM1)  !  Elevation, temperature, salinity, density
               %           END DO
               %#       else
               %           DO I=1,M
 ...
 %         varnames={  'H'; 'zeta';  'D'; 'DL'; 'DO';         'LDOC';     'ALG1';    'ALG2';     'NH4';     'NO3';     'PO4';       'T';    'S'}; 
 %         varunits={'(m)';  '(m)';'(m)';'(m)';'(mgDO/L)';'(gC/m^3)'; '(gC/m^3)';'(gC/m^3)';'(gN/m^3)';'(gN/m^3)';'(gP/m^3)';'(\circC)';'(psu)'};
         ...
         if(~(read_his_matlab || read_his_medm || read_his_nc))
            error(['Oops, wrong file extension']);
         end
          %list available variables based on file type

         if(strcmp(hfile_ext,'.nc'))
                display('Available variables to plot transects are:')
                display('1 : (u,v,ww) Eastward velocity (m/s) at element center and sigma layer center');
                display('             Northward velocity (m/s) at element center and sigma layer center');
                display('             Upward velocity (m/s) at element center and sigma layer center');
                display('2 : T     Temperature (degC) at node and sigma layer center');
                display('3 : S     Salinity (psu) at node and sigma layer center');
                display('4 : kh    Turbulent eddy diffusivity (m^2/s) at node and sigma layer surface');
                display('5 :       User defind'); 

                invalid_input=true ;
                while(invalid_input)
                   i_par=input('please indicate which parameter want to plot: (e.g. 1 for (u,v,ww)):')
                    if(i_par>5)
                       display(['must choose from 1 to 5']);
                ...
                if(i_par==5)
        	        ...
        	        while(invalid_input)
                               myvarname=[];
 			       myvarname=input('please give parameter name to plot: (e.g. sand):','s');
				if(isempty(myvarname) || ~ischar(myvarname))
					display(['oops, invalid input, must give valid variable name']);
					invalid_input=true;
				else
	                                invalid_input=false;
				end
                ...
                %load the node-to-element connectivity
         ...
         if(strcmp(hfile_ext,'.dat') || strcmp(hfile_ext,'.mat'))
                display('Available variables to plot transects are:')
                display('1 : (u,v,ww) Eastward velocity (m/s) at element center and sigma layer center');
                display('             Northward velocity (m/s) at element center and sigma layer center');
                display('             Upward velocity (m/s) at element center and sigma layer center');
                display('2 : T     Temperature (degC) at node and sigma layer center');
                display('3 : S     Salinity (psu) at node and sigma layer center'); 
                display('6 : SEDC  Sediment concentration (kg/m^3) at node and sigma layer center');

                invalid_input=true ;
                while(invalid_input)
                    i_par=input('please indicate which parameter you want to plot: (e.g. 1 for (u,v,ww)):')
                    if(i_par ==6 || (i_par>=1 && i_par <=3)) %only 1, 2,3, 6 is accepted
                       invalid_input=false;
                    else
                       display(['must choose from 1 to 3 or 6']);
                       display(['Please try again']);
                       invalid_input=true;
                    end
                end

                if(i_par==6)  %for sedimet concentration, need to find correct sediment medm output
                              %and choose a sediment constituent, also number of bed layers
                   hfile_prefix_sed=input('please give sedimet output file name (e.g. psm_sed):','s');
                   %also run a check to make sure files exist
                   for nf=his_fn_start:his_fn_int:his_fn_end
                       hfile_sed=[his_dir '/' hfile_prefix_sed  num2str(nf,'%04d') hfile_ext];
                       if(~exist(hfile_sed,'file'))
                          error(['Oops, cannot find file ' hfile_sed ]);
                       end 
                   end
                   ...
                   while(invalid_input)
                       ...
                       nbed=0;
                       ised=0;
                       nsed=input('please give number of sediment constituents (e.g. 1):');
                       nbed=input('please give number of bed layers (e.g. 1):');
                       ised=input('please give index of sediment constituent to use (e.g. 1):'); 
                       if(nsed >0 && ised <=nsed && ised >=1)
                          invalid_input=false;
                       else
                          display(['[nsed, nbed, ised]=[' int2str(nsed) ',' int2str(nbed) ',' int2str(ised) ']']);
                          display(['incorrect nsed, nbed,ised values, please try again']);
                          invalid_input=true;
                       end
                   end
                end
         ...
         %add flag to calculate flux of a chosen quantity if i_par==1 (u,v,w), if no, it calculates transport (volume flux of bulk seawater and freshwater) 

         if(i_par==1)
            yesno_tracer_flux=[];
            calculate_tracerflux=0;
            invalid_input=true ;
            display(['You will be asked to choose whether or not to calculate tracer flux, if choose no, then freshwater flux will be calculated']); 
            while(invalid_input)
              yesno_tracer_flux=input('please indicate whether or not want to calculate tracer flux through transect (e.g. y/n/yes/no):','s');
              if(lower(yesno_tracer_flux(1:1))~='y' && lower(yesno_tracer_flux(1:1))~='n')
                 display('oops, incorrect input format');
                 invalid_input= true;
              else
                 invalid_input=false;
              end
            ...
            if(calculate_tracerflux)
               ...
               invalid_input=true ;
               while(invalid_input)
                    tracername=input('please give the tracer name (e.g. temp, salinity for netcdf outputs, T , S for medm outputs):','s');
                    if(~ischar(tracername) || isempty(tracername))
                       invalid_input=true;
                    else
                       invalid_input=false;
                       if(read_his_medm && (lower(tracername)~='t' && lower(tracername)~='s'))   %can only do T and S with medm files
                          display(['oops, for medm history output files, tracer name should be T or S']);
                          invalid_input=true;
                       end
                    end
               end
            else  %by default tracer name is salt and will use it to calculate fresh water flux
               if(read_his_medm)
	               tracername='S';         
               end
               if(read_his_nc)
                       tracername='salinity';  %FVCOM uses 'salinity'  in netcdf files
               end
            end
         end

         %add flag to calculate runmean  (to finish coding ...) 

         %----

           climits(1)=input('please input minimum color value for the parameter (e.g. 0 for salt, -1 for velocity):')
           climits(2)=input('please input maximum color value for the parameter (e.g. 35 for salt, 1 for velocity):')
           ...
           zlimit(1)=inf;
           zlimit(2)=-inf;
           zlimit(2)=input('Please input maximum z coorinate value (m) (e.g. 2 for 2 meter above MSL):');

           %plot a grid and let user choose transect

           transname=input('please give a transect name (no spaces please, e.g. mainbasin):','s');

           %let user choose whether or not to plot matlab figures
           yesno_matlabfig=1;
           yesno_matlabfig=input('Please indicate whether yes or not to plot matlab figures (0-no,1-yes):');


     %get information of the transect 

         qa=dir(['trans_loc_' transname '.mat']);   %see if there is a past transect saved

         if (isempty(qa))  %create transect by clicking through the map interactively
...
%                          Replace middle-click with Command+click
                 ...
                 ds_transect=input('please input segement length (m) for subsetting transect (e.g. 500):');
         ...
         else %load previous transect if already made one in past 
                 load(['trans_loc_' transname '.mat']);   %load the transect if already have one
                 %plot the transect too if plots not already existing
              ...
              for i=1:length(xp)
                  ...
                  for j=1:NE   %check elements one by one
                    ...
                    %this inpolygon is kindaslow
                    %     if(inpolygon(xp(i),yp(i),x_n(e2n(j,2:4)),y_n(e2n(j,2:4)))==1) %check if xp, yp is in the any of element
                    %         isgood(i)=1;
                    %     end

                       %use a faster InPolygon() c function instead:
                       [in_on,on,strict] = InPolygon(xp(i),yp(i),x_n(e2n(j,2:4)),y_n(e2n(j,2:4)));
                       if(in_on==1)   %if in or on an element j, then mark as good and record the element j and nodes e2n(j,2:4)
                           isgood(i)=1;  %
                           NE_use=NE_use+1;
            ...
            %--------------
               ...
               %debug-------
               %if(make_transectplot)
               %  plot(x_e(IE_use_uniq),y_e(IE_use_uniq),'ko','markersize',10,'markerfacecolor',[ 0 0 0]) ;
               %  plot(x_n(IN_use_uniq),y_n(IN_use_uniq),'kp','markersize',10,'markerfacecolor',[ 1 0 0]) ; 
               %end
               %------------
            ...
            nx_p=cos(angle_trans+pi/2.0); %normal vector of the transect in (x,y) coordinate (90 deg counter-clociwise from tangential direction)
            ...
            if(make_transectplot)
               ...
               saveas(gcf,[tmpdirname './' transname '_transect.eps'], 'epsc2');
               saveas(gcf,[tmpdirname './' transname '_transect.png'], 'png');
               saveas(gcf,[tmpdirname './' transname '_transect.fig'], 'fig');
            ...
            %display(['debugging pause, Ctrl-C to stop and debug']);
            %pause  %debug

            %to speed up calculation, we should find all elements that tightly embrace the transect
            %and only use those grids data information for interpolation and other calculations

            display(['Please wait while loading files']);
      ...
      switch(i_par)
            ...
            case  2  %temperature
                tdata2d.Nvar=4;
                tdata2d.varnames={'r','z','zdummy','T'};
                
                tdata3d.Nvar=5;
                tdata3d.varnames={'x','y','z','T','depth'};

                tdata2d_timeavg.Nvar=4;
                tdata2d_timeavg.varnames={'r','z','zdummy','T'};

                tdata3d_timeavg.Nvar=5;
                tdata3d_timeavg.varnames={'x','y','z','T','depth'};

                %runmean

           case  3  %salt
               tdata2d.Nvar=4;
               tdata2d.varnames={'r','z','zdummy','S'};

               tdata3d.Nvar=5;
               tdata3d.varnames={'x','y','z','S','depth'};

               tdata2d_timeavg.Nvar=4;
               tdata2d_timeavg.varnames={'r','z','zdummy','S'};

               tdata3d_timeavg.Nvar=5;
               tdata3d_timeavg.varnames={'x','y','z','S','depth'};

               %runmean

           case  4  %turbulent diffusivity
               tdata2d.Nvar=4;
               tdata2d.varnames={'r','z','zdummy','kh'};
               
               tdata3d.Nvar=5;
               tdata3d.varnames={'x','y','z','kh','depth'};

               tdata2d_timeavg.Nvar=4;
               tdata2d_timeavg.varnames={'r','z','zdummy','kh'};

               tdata3d_timeavg.Nvar=5;
               tdata3d_timeavg.varnames={'x','y','z','kh','depth'};
           ...
           case 5  %user defined 
               ...
               tdata2d.Nvar=4;
               tdata2d.varnames={'r','z','zdummy',myvarname};

               tdata3d.Nvar=5;
               tdata3d.varnames={'x','y','z',myvarname,'depth'};

               tdata2d_timeavg.Nvar=4;
               tdata2d_timeavg.varnames={'r','z','zdummy',myvarname};

               tdata3d_timeavg.Nvar=5;
               tdata3d_timeavg.varnames={'x','y','z',myvarname,'depth'};
            ...
            case 6 %sediment concentration
               ...
               tdata2d.Nvar=4;
               tdata2d.varnames={'r','z','zdummy','sed'};

               tdata3d.Nvar=5;
               tdata3d.varnames={'x','y','z','sed','depth'};

               tdata2d_timeavg.Nvar=4;
               tdata2d_timeavg.varnames={'r','z','zdummy','sed'};

               tdata3d_timeavg.Nvar=5;
               tdata3d_timeavg.varnames={'x','y','z','sed','depth'};
   ...
   %loop through all file sand read the data and 
   %then interpolate onto transect,
   %then calculate flux through transect
           ...
           %read history output file
            ...
            if(read_his_nc)            %open netcdf file
              ...
              switch(i_par)
                    case  1  %three components of u,v,w
                         ...
                         if(prod(size(par4))==0)
                           error(['oops, not able to read variable ' tracername ' in file ' hfile, ' or it is empty']); 
                         end
			...
			parname=myvarname;
              ...
              %save data into matlab format for later use
               ...
               tmp_matfile=[tmpdirname  'fvcom_hisdata_' num2str(nf,'%04d') '.mat'];
            ...
            if(read_his_medm)
               ...
               command_line=['fvcom_medm2txt ' hfile ' ' tmp_file_name '  thour ' num2str(his_Nz+1)];
               eval(['!' command_line]);
               ...
               command_line=['fvcom_medm2txt ' hfile ' zeta_tmp_tmp.txt el ' num2str(his_Nz+1)];
               ...
               switch(i_par)
                    case  1  %three components of u,v,w
                         ...
                         command_line=['fvcom_medm2txt ' hfile ' ' tmp_file_name '  u ' num2str(his_Nz+1)];
                         ...
                         command_line=['fvcom_medm2txt ' hfile ' ' tmp_file_name '  v ' num2str(his_Nz+1)];
                         ...
                         command_line=['fvcom_medm2txt ' hfile ' ' tmp_file_name '  w ' num2str(his_Nz+1)];
                         ...
                         command_line=['fvcom_medm2txt ' hfile ' ' tmp_file_name ' ' lower(tracername) ' ' num2str(his_Nz+1)];
                         eval(['!' command_line]);  %call fortran program fvcom_medm2txt to dump the variable tracername in a txt
	                         if(exist(tmp_file_name,'file'))
                    ...
                    case  2  %temperature
                         ...
                         command_line=['fvcom_medm2txt ' hfile ' ' tmp_file_name '  t ' num2str(his_Nz+1)];
                         eval(['!' command_line]);  %call fortran program fvcom_medm2txt to dump the variable el in a txt
                         if(exist(tmp_file_name,'file'))
                            par1=load(tmp_file_name);
                            par1=par1';
                    ...
                    case  3  %salt
                         ...
                         command_line=['fvcom_medm2txt ' hfile ' ' tmp_file_name '  s ' num2str(his_Nz+1)];
                         eval(['!' command_line]);  %call fortran program fvcom_medm2txt to dump the variable el in a txt
                         if(exist(tmp_file_name,'file'))
                            par1=load(tmp_file_name);
                            par1=par1';
  ...
  %Now only read matlab format data 
   for nf=his_fn_start:his_fn_int:his_fn_end
          if(read_his_matlab==true) 
            ...
            hfile=[tmpdirname 'fvcom_hisdata_' num2str(nf,'%04d') '.mat'];
          end
          if(~exist(hfile,'file'))
            error(['Oops, cannot find file ' hfile ]);
          end
          ...
          if(nf==his_fn_start)  %inquire plotting interval if the first file
                invalid_input=true ;
                while(invalid_input)
                    t_int =input('please indicate time interval of processing: (e.g. 10 for every 10 records):');
                    if(t_int<1 || t_int >his_NT)
                       display(['Oops, interval must be of range : 1<= interval <=' num2str(his_NT)]);
                       display(['Please try again']);
                       invalid_input=true;
                    else
                       invalid_input=false;
         ...
         %get NT_all if interval is t_int within each file
         NT_all=NT_all+length((1:t_int:his_NT));
...
%                el_path=griddata(x_n,y_n,zeta(it,:),xp,yp);           %interpolate surface layer depth to path
                 zlimit(1)=min(zlimit(1),  min(-hpath(Ipath_in)));
%                zlimit(2)=max(zlimit(2),  max(el_path(Ipath_in)));    %interpolate surface layer depth to path

%---color limits are now from input ---
%                for k=1:his_Nz
%                   if(strcmp(parname,'velocity'))
%                      tmp_array= griddata(xy_e(:,1),xy_e(:,2),par1(it,k,:),xp(Ipath_in),yp(Ipath_in));  %interpoation of velocity on to path
%                   else
%                      tmp_array= griddata(x_n,y_n,par1(it,k,:),xp(Ipath_in),yp(Ipath_in)); 
%                   end
%                   climits(1)=min(climits(1),  min(tmp_array));
%                   climits(2)=max(climits(2),  max(tmp_array));
%                end
%------------------------------------
               end 
%               clear 'tmp_array';  
            ...
            %loop through time for the transect
            ...
            for it=1:t_int:his_NT

               it_all=it_all+1;                %global record 
               htime_all(it_all)=htime(it);    %global time
               %plot
               ...
               for k=1:his_Nz  %for kh, his_NZ is KB, for T, S, u,v,w, his_NZ is KB-1
                   ...
                   if(his_Nz==KB-1)  %sigma layer coordinate (layer is demarcated by two levels)
                       ...
                       Tz(k,:)=griddata(x_n(IN_use_uniq),y_n(IN_use_uniq), zcoord_layer(IN_use_uniq),xp,yp); %z coordinate of k'th layer (center of layer)
                       ...
                       if(length(isTznan)>=1)
                         Tz_tec(k,isTznan)=griddata(x_n(IN_use_uniq),y_n(IN_use_uniq), zcoord_layer(IN_use_uniq),xp(isTznan),yp(isTznan),'nearest'); 
                       end

                   else                             %sigma level coordinate
                       zcoord_lev=zeta(it,:)+(zeta(it,:)+h').*z(k); 
                       Tz(k,:)=griddata(x_n(IN_use_uniq),y_n(IN_use_uniq), zcoord_lev(IN_use_uniq),xp,yp); %z coordinate of k'th level
                       ...
                       isTznan=isnan(Tz_tec(k,:));  %find locations in Tz_tec that are nans
                                                    ...
                                                    %to avoid nans in Tz_tec
                       if(length(isTznan)>=1)
                         Tz_tec(k,isTznan)=griddata(x_n(IN_use_uniq),y_n(IN_use_uniq), zcoord_lev(IN_use_uniq),xp(isTznan),yp(isTznan),'nearest');
                       end
                   ...
                   if(strcmp(parname,'velocity'))
                       ...
                       isTpnan=isnan(Tp1(k,:));  %find locations in Tp1 that are nans (which means locations outside of the IE_use_uniq
                                                 %elements, and linear interpolation gives nans)

                       %for these nan places, use nearest value as extrapolation method. This happens when interpolated locations are within
                       %boundary but out of the convex hull of set of points made of all chosen element centers, because element centers are
                       %away (interior) from the boundaries by nodes. E.g., to obtain velocity on boundary nodes (positive depth), one needs to 
                       %do extrapolation. 

                       if(length(isTpnan)>=1)
                         Tp1(k,isTpnan)=griddata(xy_e(IE_use_uniq,1),xy_e(IE_use_uniq,2), squeeze(par1(it,k,IE_use_uniq)),xp(isTpnan),yp(isTpnan),'nearest');
                         Tp2(k,isTpnan)=griddata(xy_e(IE_use_uniq,1),xy_e(IE_use_uniq,2), sqeeze(par2(it,k,IE_use_uniq)),xp(isTpnan),yp(isTpnan),'nearest');
                         Tp3(k,isTpnan)=griddata(xy_e(IE_use_uniq,1),xy_e(IE_use_uniq,2), squeeze(par3(it,k,IE_use_uniq)),xp(isTpnan),yp(isTpnan),'nearest');
                       end

                       if(length(Ipath_out)>0)  %for places out of domain, still set to nans so that matlab will not plot them
                      ...
                      %then be caluclated based on v_p*concentration integrated over the transect
	                      Tp4(k,:)=griddata(x_n(IN_use_uniq),y_n(IN_use_uniq),par4(it,k,IN_use_uniq),xp,yp);  %tracer concentration
                              isTpnan=isnan(Tp4(k,:));
                              if(length(isTpnan)>=1)  %fix interpolation if interpolated to be nan (need extrapolcation)
                                  Tp4(k,isTpnan)=griddata(x_n(IN_use_uniq),y_n(IN_use_uniq),par4(it,k,IN_use_uniq),xp(isTpnan),yp(isTpnan),'nearest');
                              end
                       ...
                       Tp1(k,:)=griddata(x_n(IN_use_uniq),y_n(IN_use_uniq),squeeze(par1(it,k,IN_use_uniq)),xp,yp);       %u, T, S, kh
               ...
               %bed level values are created so that pictures are not jumping up and down because of thickness change over time
               if(KB==his_Nz+1)  %if data is on sigma layer, append bottom value as bottom level (z=-h)
 %---OLD---
 %                 Tz(his_Nz+1,:)=-hpath;                           %bottom level use fixed depth
 %---------
 ...
 %---New---
                   ...
                   Tz(k,:)=griddata(x_n(IN_use_uniq),y_n(IN_use_uniq), zcoord_bottom(IN_use_uniq),xp,yp); %z coordinate of bottom
                   ...
                   isTznan=isnan(Tz_tec(k,:));  %find locations in Tz_tec that are nans
                                                ...
                                                %to avoid nans in Tz_tec
                   if(length(isTznan)>=1)
                      Tz_tec(k,isTznan)=griddata(x_n(IN_use_uniq),y_n(IN_use_uniq), zcoord_bottom(IN_use_uniq),xp(isTznan),yp(isTznan),'nearest');
                   end
%---------
                   ...
                   if(strcmp(parname,'velocity'))
                       ...
                       Tp3(his_Nz+1,:)=Tp3(his_Nz,:);    %this is vertical velocity on bed level, we may should set it to zero for flat bottom
                                                         %however, when there is a slope, then vertical velocity can be non-zero, as 
                                                         %the only physical argument is that normal velocity should be zero for slip boundary condition
         ...
         %calculate flux 
               ...
               if(strcmp(parname,'velocity'))
                  ...
                  %find cross sectional area by taking difference in z and then in r dierction
                   ...
                   Tz_new(1,:)=griddata(x_n(IN_use_uniq),y_n(IN_use_uniq),  zcoord_lev(IN_use_uniq) ,xp,yp); %z coordinate of k'th level, k=1
                   ...
                   r_new=nan*ones([size(r,1),size(r,2)+1]);          %r=ones(his_Nz+1,length(xp)), r_new has dimension his_Nz+1 by length(xp)+1
                   ...
                   dr_trans=diff(r_new,1,2)*1000;  %first derivatinve in along transect direction (column)  (*1000 to convert from km to m)
                   ...
                   if(calculate_tracerflux)  %flux of tracer
                      flux_trans=dA_trans.*v_p(1:end-1,:).*Tp4(1:end-1,:);  %unit: (m^3/sec) * (tracer volume concentration (tracer/m^3))
                      ...
                      flux_trans=dA_trans.*v_p(1:end-1,:);                 %velocity volume  flux through each window
                     		         ...
                     		         %---Wen Long: How to calculate fresh water flux ?----
                                         ...
                                         %
                                         %   use eqn(1) and eqn (2) to find V(fresh water) with ginven salinity of bulk water S, and Smax
                                         %
                                         ...
                                         %         V_fw              is volume of fresh water needed to mix with V_Smax water of salinity Smax to produce
                      ...
                      %calculate fresh water flux (m^3/sec)
                          ...
                          %get rho_bulk(Smax) using equation of state,  where Smax is set to 34 
                                ...
                                % Equation of State: Reference: Cushman-Roisin B.,Introduction to Geophysical Fluid Dynamics, page 36, Chapter 3, 
				...
				rho_bulk_S    = 1028 * (1 - 1.7E-4 * (10 -10 ) + 7.6E-4 *(Tp4-35));  %eqn 7 assuming S=Tp4  (mixed)
                                rho_bulk_Smax = 1028 * (1 - 1.7E-4 * (10 -10 ) + 7.6E-4 *(34 -35));   %eqn 7 assuming S=Smax=34 ppt (salt)
                                rho_fw        = 1028 * (1 - 1.7E-4 * (10 -10 ) + 7.6E-4 *(0  -35));   %eqn 7 assuming S= 0 ppt  (fresh)
				...
				flux_trans_fw=flux_trans.*f_fw(1:end-1,:);    %eqn (6-1) 
                   ...
                   Igood=~isnan(flux_trans(:));  %find places that are not nans, when part of transect is outside of model domain

                   flux_total=sum(flux_trans(Igood));           %total flux is summation of all good grids on transect (m^3/sec)
                   ...
                   if(~calculate_tracerflux)
                           ...
                           flux_total_fw=sum(flux_trans_fw(Igood));     %total frewh water flux (m^3/s)
                   ...
                   Tz_min=  zlimit(1)  ;   %min(-hpath(Ipath_in));         %minimum vertical coordinate on the transect
                   Tz_max=  zlimit(2)  ;   %max(zcoord_lev(Ipath_in));     %maximum vertical coordinate on the transect (surface elevation)
                   
                   dz_vdis=(Tz_max-Tz_min)/(his_Nz);     %divide the vertical distance by same number of layers as his_Nz
                   ...
                   for k=1:Nz_vert-1
                       ...
                       if(length(Ifind>0))
                           ...
                           flux_vert(k)=sum(flux_vert_thislayer(Igood)) ; %total flux of this layer (m^3/sec)*(concentration)
                           flux_vert(k)=flux_vert(k)/dz_vdis            ; %distribution vertically (m^2/sec)*(concentration)
                 ...
                 if(yesno_matlabfig)
                   ...
                   subplot(2,1,1)
                      ...
                      %Wen Long customized quiver plotting program with u in km/s and v in m/s
                       quiverc_ukms_vms(r,Tz,u_p/1000.0,Tp3,1.2); colorbar;         %plot vectors   %u velocity is scaled to km/sec

                       if(climits(2)>0)
                         caxis([0,climits(2)]);  %for quiverc_ukms_vms plot, color is based on size of vector, and there is 
                      ...
                      title(['(u,w) on ' transname ' at time = ' num2str(htime(it),'%f8') '(days)'],'fontsize',15,'color',[0 0 1]);
                   ...
                   subplot(2,1,2)
                    ...
                    if(calculate_tracerflux)   %tracer flux
	                    ...
	                    hbar_vert=barh((Tz_vert(1:end-1)+Tz_vert(2:end))/2.0,flux_vert(1:end-1));  %horizontal bar chart
	                    title({[tracername ' flux per unit transect depth ((m^3/sec)*(conc.)/m)'];[' at time = ' num2str(htime(it),'%f8') '(days)'] } ...
                            ,'fontsize',15);
	                    xlabel('flux per unit depth ((m^3/sec)*(conc)./m)','fontsize',15);
	                    ylabel('z(m)','fontsize',15);

	                    hflux_figfilename=[tmpdirname parname,'_trans_', transname, '_', tracername,'_Qprofile_', num2str(it_all,'%010d'),'.png'];
	                    saveas(gcf,hflux_figfilename,'png');

	                    hflux_figfilename=[tmpdirname parname,'_trans_', transname, '_', tracername,'_Qprofile_', num2str(it_all,'%010d'),'.fig'];
	                    saveas(gcf,hflux_figfilename,'fig');
		    else  %volume flux of bulk sea water  and fresh water volume flux
                          subplot(2,1,1)
         	            hbar_vert=barh((Tz_vert(1:end-1)+Tz_vert(2:end))/2.0,flux_vert(1:end-1));  %horizontal bar chart
                            title({['Flux per unit transect depth ((m^3/sec)/m)'];[' at time = ' num2str(htime(it),'%f8') '(days)'] } ...
                            ,'fontsize',15);
			  ...
			  subplot(2,1,2)
                            ...
                            hbar_vert_fw=barh((Tz_vert(1:end-1)+Tz_vert(2:end))/2.0,flux_vert_fw(1:end-1));  %horizontal bar chart
                            title({['Freshwater flux per unit transect depth ((m^3/sec)/m)'];[' at time = ' num2str(htime(it),'%f8') '(days)'] } ...
                            ,'fontsize',15);
                            xlabel('flux per unit depth ((m^3/sec)/m)','fontsize',15);
                            ylabel('z(m)','fontsize',15);

                          hflux_figfilename=[tmpdirname parname,'_trans_', transname, '_Qprofile_fw_', num2str(it_all,'%010d'),'.png'];
                          saveas(gcf,hflux_figfilename,'png');

                          hflux_figfilename=[tmpdirname parname,'_trans_', transname, '_Qprofile_fw_', num2str(it_all,'%010d'),'.fig'];
                          saveas(gcf,hflux_figfilename,'fig');
                  ...
                  %2)save plot data in matlab format 
                    ...
                    %save the data into matlab as well so that it can be replotted easily
                     tmpfilename=[tmpdirname tracername '_' parname,'_trans_', transname, '_',num2str(nf,'%06d'),'_' num2str(it,'%010d'),'.mat'];
			 ...
			 %save the data into matlab as well so that it can be replotted easily
                         tmpfilename=[tmpdirname parname,'_trans_', transname, '_',num2str(nf,'%06d'),'_' num2str(it,'%010d'),'_fw.mat'];
                         save(tmpfilename,'r',          ...   % distance along transect (km)
                                      'Tz',         ...   % vertical coordinate (m)
                                      'u_p',        ...   % velocity along the transect in transect surface
                                      'Tp3',        ...   % upward velocity (w) on the transect in transect surface
                 ...
                 if(yesno_matlabfig)
                   ...
                   set(gca,'xlim',[0,max(r(:))]);
                   set(gca,'ylim',[zlimit(1),zlimit(2)+2]);  %offset zlimit(2)  by 2 meter
                   title([parname parunit ' on ' transname  ' at time = ' num2str(htime(it),'%f8') '(days)'],'fontsize',15,'color',[0 0 1]);
                 ...
                 %2) save the plot data into matlab data 
                   tmpfilename=[tmpdirname parname,'_trans_', transname, '_', num2str(nf,'%06d'),'_',num2str(it,'%010d'),'.mat'];
                   save(tmpfilename,'r',         ...   % distance along transect (km)
                                    'Tz',        ...   % vertical coordinate (m)
                                    'Tp1',       ...   % parameter value 
                                    'climits',   ...   % color limits
	    ...
	    %B) Save the matlab plot into .png and .fig files 
               ...
               if(yesno_matlabfig)
	               %save plots
	               figfilename=[tmpdirname parname,'_trans_', transname, '_',num2str(nf,'%06d'),'_', num2str(it,'%010d'),'.png'];
	               saveas(gcf,figfilename,'png');

	               figfilename=[tmpdirname parname,'_trans_', transname, '_',num2str(nf,'%06d'),'_', num2str(it,'%010d'),'.fig'];
           ...
           %C) plot in tecplot format
               ...
               if(strcmp(parname,'velocity'))  %save velocity transects
                   ...
                   tdata2d.surfaces(1).zonename=[parname ' transect ' transname, ' 2D zone file number ' num2str(nf,'%06d') ...
                   ...
                   tdata3d.surfaces(1).zonename=[parname ' transect ' transname, ' 3D zone file number ' num2str(nf,'%06d') ...
                                                                                 ' time record ' num2str(it_all,'%8d')];
                   tdata3d.surfaces(1).x=repmat(reshape(xp,[length(xp),1]),[1,size(r,1)]);  %first dimension is number of points along xp
                   ...
                   tdata3d.surfaces(1).v(5,:,:)=v_p'; % 'v_p' (m/s) horizontal velocity component that is perpendicular to transect
                   tdata3d.surfaces(1).v(6,:,:)=Tz';  %last variable in v is -depth
                   tdata3d.surfaces(1).order=2;       %surface is IK-odered (I is first dimension of x, y, z, v, K is 2nd dimension of of x, y, z, v)
                   ...
                   tdata3d.FEsurfaces(1).zonename=['PSWQM 2.0 grid file number ' num2str(nf,'%06d') ' time record ' num2str(it_all,'%8d')];
                   ...
                   tdata3d.FEsurfaces(1).zonename=['PSWQM 2.0 grid time record file number ' num2str(nf,'%06d') ' time record' num2str(it,'%8d')];
                   tdata3d.FEsurfaces(1).x=x_n;   %x of all nodes
                   tdata3d.FEsurfaces(1).y=y_n;   %y of all nodes
                   tdata3d.FEsurfaces(1).z=-h_n;  %z of all nodes is given as -h
                   tdata3d.FEsurfaces(1).order=3; %order 3, surface is defined on (x, y) coordinates
               ...
               %save the tecplot data into matlab file first, will load them after the nf loop
                tmpfilename=[tmpdirname parname,'_trans_', transname, '_tecplot_', num2str(nf,'%06d') '_' num2str(it,'%010d'),'.mat'];
                save(tmpfilename,'tdata2d', ...
                                 'tdata3d', ...
                                 '-mat');

            end     %end of time loop of this file  (it)
     end  %end of file loop (nf)

 %Aggregrate all all tecplot plots of individual time records into a big tecplot history file
 %so that it can be played as an animation in tecplot
     ...
     tecfile2d=[tmpdirname parname,'_trans_', transname,'_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'), '_2D.plt'];
     mat2tecplot(tdata2d_all,tecfile2d);

     tecfile3d=[tmpdirname parname,'_trans_', transname,'_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'), '_3D.plt'];
 ...
 %Create a time average for the 2D and 3D zones 
     ...
     tdata3d_timeavg.surfaces(1).order=2;    %surface is IK-odered (I is first dimension of x, y, z, v, K is 2nd dimension of of x, y, z, v)
                                             %surfaces are defined on (x,z) plane

     %add bathymetry zone
     tdata3d_timeavg.FEsurfaces(1).zonename=['CHESFVM 1.0 '];
     ...
     tecfile2d_timeavg=[tmpdirname parname,'_trans_', transname,'_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'), '_2D_timeavg.plt'];
     mat2tecplot(tdata2d_timeavg,tecfile2d_timeavg);

     tecfile3d_timeavg=[tmpdirname parname,'_trans_', transname,'_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'), '_3D_timeavg.plt'];
 ...
 %Finally save everything about the tecplot data  in a matlab file for future inspection
     save([tmpdirname parname '_' transname '_calculation_' num2str(his_fn_start,'%06d') '_' num2str(his_fn_end,'%06d') '.mat'], ...
 ...
 %
	 ...
	 %1) plot and save time series of total flux records
	  ...
	  figfilename=[tmpdirname parname,'_trans_', transname, '_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'),'_flux','.png'];
	  saveas(gcf,figfilename,'png');

	  figfilename=[tmpdirname parname,'_trans_', transname, '_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'),'_flux','.fig'];
	  saveas(gcf,figfilename,'fig');
	 ...
	 %2) plot vertical distribution of time averaged flux
	  ...
	  hbar_vert=barh((Tz_vert(1:end-1)+Tz_vert(2:end))/2.0,mean(flux_vert_all(:,1:end-1),1));  %horizontal bar chart of time averaged flux
	  title({['Time averaged flux per unit transect depth ((m^3/sec)*(conc.)/m)'];...
		 ['time=[' num2str(htime_all(1),'%f8')  ','  num2str(htime_all(end),'%f8') '] (days)'] } ...
		 ,'fontsize',15);
	  xlabel('<flux per unit depth> ((m^3/sec)*(conc.)/m)','fontsize',15);
	  ylabel('z(m)','fontsize',15);
	  hflux_figfilename=[tmpdirname parname,'_trans_', transname, '_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'),'_Qprof_avg','.png'];
	  saveas(gcf,hflux_figfilename,'png');
	  hflux_figfilename=[tmpdirname parname,'_trans_', transname, '_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'),'_Qprof_avg','.fig'];
	  saveas(gcf,hflux_figfilename,'fig');
	  ...
	  if(~calculate_tracerflux)  %plot fresh water flux time series and vertical distribution
		 ...
		 %plot and save time series of total flux
		  ...
		  xlabel('Time(days)','fontsize',15);
		  figfilename=[tmpdirname parname,'_trans_', transname, '_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'),'_fw_flux','.png'];
		  saveas(gcf,figfilename,'png');

		  figfilename=[tmpdirname parname,'_trans_', transname, '_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'),'_fw_flux','.fig'];
		  saveas(gcf,figfilename,'fig');
		 ...
		 %plot vertical distribution of time averaged flux
		  ...
		  hbar_vert=barh((Tz_vert(1:end-1)+Tz_vert(2:end))/2.0,mean(flux_vert_all_fw(:,1:end-1),1));  %horizontal bar chart of time averaged flux
		  ...
		  hflux_figfilename=[tmpdirname parname,'_trans_', transname, '_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'),'_fw_Qprof_avg','.png'];
		  saveas(gcf,hflux_figfilename,'png');
		  hflux_figfilename=[tmpdirname parname,'_trans_', transname, '_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'),'_fw_Qprof_avg','.fig'];
		  saveas(gcf,hflux_figfilename,'fig');
	  ...
	  if(calculate_tracerflux) %tracer flux
		  ...
		  datafilename=[tmpdirname parname,'_trans_', transname, '_',tracername,'_Qprof.mat'];
		 ...
		 save(datafilename,'htime_all','transname','flux_all','Tz_vert','flux_vert_all','Ipath_in','Ipath_out','xp','yp','readmetxt','-mat');

	 else  %volume and freshwater volume flux

		  %save the flux data into a matlab file
		  datafilename=[tmpdirname parname,'_trans_', transname, '_fw_Qprof.mat'];
		 ...
		 save(datafilename,'htime_all','transname','flux_all','flux_all_fw','Tz_vert','flux_vert_all','flux_vert_all_fw', ...
	 ...
	 tecfile_TS=[tmpdirname parname,'_trans_', transname, '_', num2str(his_fn_start,'%06d'), '_', num2str(his_fn_end,'%06d'),'_flux_timeseries.plt'];
	 ...
	 for it=1:length(htime_all)
	    if(calculate_tracerflux)
		    tdata_Prf.lines(it).zonename=[tracername ' flux zone of transect ' transname ' at time ' num2str(htime_all(it),'%8.4f')];
	    else
		    tdata_Prf.lines(it).zonename=['volume flux zone of transect ' transname ' at time ' num2str(htime_all(it),'%8.4f')];
	    end
	 ...
	 if(calculate_tracerflux)  %tracer flux
	     tecfile_Prf=[tmpdirname parname,'_trans_', transname, '_', tracername, '_',  num2str(his_fn_start,'%06d'), ...
			  '_', num2str(his_fn_end,'%06d') '_fluxprofile_timeseries.plt'];
	 else  %volume flux
	     tecfile_Prf=[tmpdirname parname,'_trans_', transname, '_volume_', num2str(his_fn_start,'%06d'), ...
			  '_', num2str(his_fn_end,'%06d') '_fluxprofile_timeseries.plt'];
	 ...
	 if(~calculate_tracerflux)   %also do fresh water flux
	     ...
	     for it=1:length(htime_all)
	       tdata_Prf_fw.lines(it).zonename=['freshwater flux zone of transect ' transname ' at time ' num2str(htime_all(it),'%8.4f')];
	     ...
	     tecfile_Prf_fw=[tmpdirname parname,'_trans_', transname, '_volume_fw_', num2str(his_fn_start,'%06d'), ...
			       '_', num2str(his_fn_end,'%06d') '_fluxprofile_timeseries.plt'];
	 ...
	 if(calculate_tracerflux) %tracer flux
		 tdata_Prf_mean.lines(1).zonename=[tracername ' mean flux zone transect ' transname ' during time from ' ... 
			    num2str(htime_all(1),'%8.4f') ' to ' num2str(htime_all(end),'%8.4f')];
	 else  %volume flux
		 tdata_Prf_mean.lines(1).zonename=['volume time mean flux zone transect ' transname ' during time from ' ...
			    num2str(htime_all(1),'%8.4f') ' to ' num2str(htime_all(end),'%8.4f')];
	 ...
	 if(calculate_tracerflux)
		tecfile_Prf_mean=[tmpdirname parname,'_trans_', transname,'_' tracername '_', num2str(his_fn_start,'%06d'), ...
				   '_' num2str(his_fn_end,'%06d'), '_fluxprofile_mean.plt'];
	 else
		tecfile_Prf_mean=[tmpdirname parname,'_trans_', transname,'_volume_', num2str(his_fn_start,'%06d'), ...
				   '_' num2str(his_fn_end,'%06d'), '_fluxprofile_mean.plt'];
	 end
	 ...
	 if(~calculate_tracerflux)  %fresh water time  mean flux vertical distribution
	   ...
	   tdata_Prf_mean_fw.lines(1).zonename=['fresh water mean flux zone transect ' transname ' during time from ' ...
					    num2str(htime_all(1),'%8.4f') ' to ' num2str(htime_all(end),'%8.4f')];
	   ...
	   tdata_Prf_mean_fw.lines(1).y=(mean(flux_vert_all(:,1:end-1),1))';        %flux after time average (m^3*(conc.)/sec)
									       %transpose from it's row to column
	   tecfile_Prf_mean_fw=[tmpdirname parname,'_trans_', transname,'_volume_fw_', num2str(his_fn_start,'%06d'), ...
			        '_' num2str(his_fn_end,'%06d'), '_fluxprofile_mean.plt'];
