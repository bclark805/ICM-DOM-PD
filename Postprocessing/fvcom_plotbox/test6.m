...
%             his2flux.plotoutdir        =['./' his2flux.transname '/'];  %output plt dirname
%             %fvcom_his2flux_fv(his2flux);
%             int_plot=10;  %plot every same records
%             test5
%
...
%                                                               TCE_edge_use_uniq,TCE_use_uniq);
		...
		[xflow_fw_tce_edge_advection,xflow_fw_tce_edge_diffusion, ...
	       ...
	       %---debug----
	          ...
	          figure;
                  ...
                  for itce=1:length(TCE_edge_use_uniq(:))
                      ...
                      %also show the TCE flux value near that TCE's ia point
                      text_x=ia_x+0.3*mid_length*cos(mid_angle);
                      text_y=ia_y+0.3*mid_length*sin(mid_angle);
                      text(text_x,text_y,num2str(xflux_tce_edge(itce_edge)),'color',[1 0 0]);
			   ...
			   %next time step
				...
				%calculate the total mass
			     ...
			     %get the flux on the cutting cord using mass balance
					...
					for k=1:KBM1
                                           c flux_in_vertical(k)=wts(itce,k+1)*area_cutout;  % (m^3/sec) %inflow from bottom
				    ...
				    % flux_in_horizontal is -cflow{itce}{iseg}
                             ...
                             cflow_cutcord{itce}{iseg}	    = -cflow{itce}{iseg} ...
							      ...
							      +sum(cflux_in_vertical - cflux_out_vertical);  %summation of vertical flux should cancel
			     ...
			     %                                                   i=1
					...
					tflux_in_vertical_advection=zeros([1,KBM1]); 
					...
					for k=1:KB
					    ...
					    elseif (k==KB)
						c_tce_level(k)=c_tce(itce,KBM1)			%bottom level
                                            else
						c_tce_level(k)=(c_tce(k)+c_tce(k-11))*0.5;	%level 2 to KB-1
                                            end
					end
				    ...
				    %calculate flux_vertical_diffusion (kh*dS/dz=(kh/D) * (dS/dsigma)), D=h_n+z_n is total depth , dz= D*dsigma
					...
					for k=1:KBM1
					    if(k==1)
						    ...
						    tflux_out_veritical_diffusion(k) = kh(itce,k+1)*(c_tce(k)-c_tce(k+1)) ...
										     /level_thickness;
                                            elseif(k==KBM1)
						    ...
						    tflux_in_vertical_diffusion(k) =  kh(itce,k)*(c_tce(k-1)-c_tce(k))	...
                                                                                     /level_thickness;
						    tflux_out_veritical_diffusion(k) = 0.0 %bottom diffusive tracer flux set to zero (dS/dz=0)
											  %of buttom layer 
					    else
                                                    level_thickness = (h_n(itce)+zeta_n(itce))*( ( z(k) +z(k-1))/2.0	...  %layer k-1
                                                                                                -( z(k) +z(k+1))/2.0	...  %layer k
			      ...
			      %                                       = -tflux_out_veritical_diffusion/S0    %= - tracer_diffusive_flux/S0
                              %
			      ...
			      f_fw_top=(c_tce_level(1:KBM1)-Smax)/Smax;		    %fraction of advective freshwater flow on top of layers 1:KBM1
			      f_fw_bottom=(.*c_tce_level(2:KB)-Smax)/Smax;	    %fraction of advective freshwater flow on bottom of layers 1:KBM1
    
                              cflow_cutcord_fw{itce}{iseg} = -cflow_fw{itce}{iseg}				... %fresh water flow coming 
													     	    %in from sides (advective+diffusion)
							    ...
							    -sum(-tflux_out_vertical_diffuion./Smax)		... %diffusive fw flux out of top
                              ...
                              cflow_cutcord_fw_prf{itce}{iseg} = -cflow_fw_prf{itce}{iseg}			    ...
								 ...
								 -(cflux_out_vertical.*c_tce_level(1:KBM1))         ...
                                                                 +(cflux_in_vertical .*c_tce_level(2:KB))           ...
	                                                         -(-tflux_out_vertical_diffuion./Smax)              ...
